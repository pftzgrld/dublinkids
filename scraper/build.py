...
def main():
    ...
    # First source wins, EXCEPT that booking-required evidence from a later
    # duplicate upgrades an assumed drop-in — the libraries listing defaults
    # to drop-in when the page shows no booking text, while dlr-events often
    # states 'Booking is needed' explicitly for the same event.
    seen, out = {}, []
    for r in rows:
        ...
        if key in seen:
            kept = seen[key]
            if kept["book"] == "Drop-in" and \
                    r["book"] in ("Book online", "Contact branch"):
                kept.update(book=r["book"], link=r["link"], dropin=False)
                if kept["status"] == "No booking needed":
                    kept["status"] = r["status"]
                    kept["ok"] = r["ok"]
            continue
        seen[key] = r
        out.append(r)
