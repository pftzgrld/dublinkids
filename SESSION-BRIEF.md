...
| dlrevents | dlr county events — the MAIN council site's system (arts/heritage/parks family programme) | dlrcoco.ie/dlr-events, ?page=N pagination + detail pages | overlaps the libraries listing — src_dlr stays first; booking-evidence merge rule in build.py; filter on the event's own description (footer promotes other events) |
...
| heritageweek | National Heritage Week, every organiser in the four Dublin councils + north Wicklow (cathedrals, castles, Chester Beatty, Dead Zoo, Martello towers, private houses) | heritageweek.ie/event-listings, `where[0]=` + `eventFeatures[0]=` GET filters, 12 cards/page at `/pN`, then detail pages | seasonal — 9 days each August, [] the rest of the year (the failure guard then keeps the past rows, which is fine). Organiser tags do the first cut: children-tagged kept outright, family-tagged must also carry a kid word in an EVENT-FRAMING phrase (boilerplate 'children must be accompanied' / 'children 4–16: €6' stripped first). A children-tagged plain tour with nothing for kids in the text is a mis-tag and is dropped (Scientology's Georgian house tour). Booking links are live-checked — a dead organiser link (ifi.ie/familyfest 404s) falls back to the listing page. Wicklow cut to the north by eircode key (A98/A63/A67) — Blessington and Russborough are out |
